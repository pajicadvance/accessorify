...
public class TotemOfFreezingAccessory implements Accessory {
    ...
    public static void init() {
        RegistryEntryAddedCallback.event(BuiltInRegistries.ITEM).register((i, rl, item) -> {
            if (rl.equals(ResourceLocation.parse("friendsandfoes:totem_of_freezing"))) {
                //? if <= 1.21.1
                AccessoriesAPI.registerAccessory(item, new TotemOfFreezingAccessory());
                //? if > 1.21.1
                /*AccessoryRegistry.register(item, new TotemOfFreezingAccessory());*/
            }
        });
    }
